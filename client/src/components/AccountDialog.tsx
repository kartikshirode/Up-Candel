    ...
    <Dialog title="Account" onClose={onClose}>
      <div className="flex flex-col gap-4">
        ...
        <label className="flex items-start gap-3">
          <input type="checkbox" className="mt-0.5 h-4 w-4 accent-[var(--color-chalk)]" checked={charges} onChange={(e) => setCharges(e.target.checked)} />
          <span>
        ...
        {charges !== portfolio.account.chargesEnabled && (
          <button
            ...
            className="h-9 self-start rounded-[2px] border border-line bg-raised px-3 font-medium hover:border-muted"
        ...
        <div className="border-t border-line pt-4">
          ...
          <label className="mt-3 flex flex-col gap-1">
            ...
            <input inputMode="numeric" value={capital} onChange={(e) => setCapital(e.target.value.replace(/[^\d]/g, ""))} className="h-9 rounded-[2px] border border-line bg-raised px-2.5 num" />
          </label>
          {!confirming ? (
            <button
              disabled={!capitalOk}
              onClick={() => setConfirming(true)}
              className="mt-3 h-9 rounded-[2px] border border-down/60 px-3 font-medium text-down hover:bg-down/10 disabled:opacity-40"
            ...
            <div className="mt-3 flex items-center gap-2">
              <button
                ...
                className="h-9 rounded-[2px] bg-down px-3 font-semibold text-white"
